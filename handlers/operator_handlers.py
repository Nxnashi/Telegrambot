...
{rating_text if rating_text else "  Нет оценок"}"""
    ...
    def handle_call(call):
        ...
        if data.startswith("process_"):
            ...
            status, current_operator = result[0], result[1]

            if status == "Заявка в процессе":
                bot.answer_callback_query(call.id, f"Уже взял: {current_operator}", show_alert=True)
                return

            if status == "Выполнено":
                bot.answer_callback_query(call.id, "Заявка уже выполнена", show_alert=True)
                return

            cursor.execute("""
                UPDATE requests SET status = 'Заявка в процессе', operator_name = ?
                WHERE id = ?
            """, (operator_name, request_id))
            conn.commit()

            original_text = call.message.caption or call.message.text or ""
            update_operator_message(bot, request_id, original_text, "Заявка в процессе", operator_name)
            ...
            for operator_id, message_id in cursor.fetchall():
                try:
                    if operator_id == call.from_user.id:
                        done_markup = types.InlineKeyboardMarkup()
                        done_markup.add(types.InlineKeyboardButton(
                            "✔️ Выполнено", callback_data=f"done_{request_id}"
                        ))
                        bot.edit_message_reply_markup(operator_id, message_id, reply_markup=done_markup)
                    else:
            ...
            open_chat(request_id, user_id, call.from_user.id)
        ...
        # =========================
            ...
            request_id = data.split("_")[1]
            clicked_operator = call.from_user.first_name
            ...
            if status == "Заявка отправлена":
                bot.answer_callback_query(call.id, "Сначала возьмите заявку в работу", show_alert=True)
                return

            if current_operator and current_operator != clicked_operator:
                bot.answer_callback_query(call.id, f"Заявка закреплена за {current_operator}", show_alert=True)
                return
            ...
            original_text = call.message.caption or call.message.text or ""
            update_operator_message(bot, request_id, original_text, "Выполнено", current_operator)

            bot.answer_callback_query(call.id)
            bot.send_message(call.message.chat.id, f"✅ Заявка #{request_id} выполнена")
    ...
    # CHAT: OPERATOR → USER
    # =========================
    ADMIN_BUTTONS = ["📊 Статистика операторов", "📥 Экспорт заявок в Excel", "📋 Активные заявки"]
